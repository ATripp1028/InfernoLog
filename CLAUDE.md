...
- `User.cognitoSub` is **legacy and read by nothing**. It is still written alongside the identity (`createUserForSignup`, `provisionE2eUser.ts`) until the column is dropped. Don't add a new read of it.
