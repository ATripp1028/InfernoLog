...
- Cognito User Pool federates with Google. **A `users` row is reached through `AuthIdentity`, not a column on `users`**: each identity is one external account connected to the user, and an account can hold several. Every lookup from a token's sub goes through that table.
...
- `User.cognitoSub` and `User.discordId` are **legacy and read by nothing**. They are still written alongside the identity (`createUserForSignup`, `provisionE2eUser.ts`, the Discord link routes) until the columns are dropped; `/v1/me` still sends a `discordId`, derived from the identities, for frontends deployed before `identities` existed. Don't add a new read of either column.
