...
- Cognito User Pool federates with Google. **A `users` row is reached through `AuthIdentity`, not a column on `users`**: each identity is one sign-in method (its own Cognito user, keyed by `AuthIdentity.cognitoSub`), and an account can hold several. Every lookup from a token's sub goes through that table.
- `User.cognitoSub` is **legacy and read by nothing except the postAuthentication trigger**. It is still written alongside the identity (`createUserForSignup`, the trigger's backfill, `provisionE2eUser.ts`) so the switch to `AuthIdentity` can be rolled back, and is dropped once the trigger goes. Don't add a new read of it.
