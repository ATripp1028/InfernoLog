...
| `enjoyment`                | No       | 0-100 whole numbers. A value ≤ 10 is read as an older 0-10 export and multiplied by 10                                                                                                                                                      |
...
| `fps`            | No       | Integer                                                                                                             |
| `device`         | No       | pc or mobile                                                                                                        |
| `enjoyment`      | No       | 0-100 whole numbers (≤ 10 is read as an older 0-10 export and ×10)                                                  |
...
- **A beaten level takes no session dated after its completion.** Beat a level on the 4th and a session dated the 2nd (or the 4th) imports normally — that is backfill, which is what this tab is for. A session dated the 6th is refused and reported as skipped: a completion is the end of that level's history, so a later session would contradict it. To re-import a level's history with different dates, delete the level's data first. The rule compares dates only, so an undated session (or an undated completion) leaves nothing to violate and imports. The logging endpoints enforce the same rule through the same comparator — see `LOGGING_FLOW_RECONCILIATION.md`.
...
- **Formatting is client-side**: dates in the user's `date_format_preference`, scores on the 0–10 scale (internal `0-100 ÷ 10`, which round-trips through the importer's ≤10 rule), enjoyment on 0–100 (internal, unconverted — it is shown on that scale in the app), coin bitmask → `coin_1/2/3`, enum casing lowered.
