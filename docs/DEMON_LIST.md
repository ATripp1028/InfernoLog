...
Only progress updates with `kind = completion` appear on the demon list by default. A v2 toggle allows non-completion entries to appear alongside completions.
...
(and the `ClassicDemonList` comment in `schema.prisma`) called it "the rebalancing
...
- **Place now:** opens the demon list. With a list reference, the view is pre-scrolled to the matching tier spot (highest match, or just above the closest under). Without a reference, it opens at the top and the user scrolls to place.
...
Every write that touches `classic_demon_list.ranking_index` records what it did in
...
the current `classic_demon_list` table is not consulted. Order by `(createdAt,
...
no current position. Reads `classic_demon_list` plus completion dates; the event log
