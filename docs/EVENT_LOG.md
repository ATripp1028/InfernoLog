...
| `rating_rank`      | Its 1-based position in the user's rating order, before and after. That order is defined once, in `packages/core/src/ratingOrder.ts`, and is the same one the Ranking page renders — see `RATING_SYSTEM.md` → "The Canonical Rating Order" |
