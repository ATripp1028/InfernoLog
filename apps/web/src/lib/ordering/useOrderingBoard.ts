// The two-column ordering board: the containers' live ordering during a drag,
// the cross-container rules, and which write a completed drag turns into
// (place, reorder, unplace, or nothing).
//
// Shared by the demon list and the MANUAL rating ranking, which are the same
// board on different axes — one orders completions by difficulty, the other by
// quality, and the drag arithmetic does not care which. What differs is only
// the three writes, which the caller injects.
//
// Kept out of the components so the arithmetic can be exercised without
...
import type { ContainerId, OrderedItem } from './types'

/**
 * The two columns, in whichever ordering the caller is running.
 *
 * Generic over the row types so each caller gets its own shape back out of the
 * filtered views — the demon list's entries carry a tier badge and a listIndex
 * the ranking has no use for, and neither should have to widen to the other.
 */
export interface OrderingData<
  P extends OrderedItem & { rank: number },
  U extends OrderedItem,
> {
  placed: P[]
  unplaced: U[]
}
...
/**
 * The three writes a drag can turn into.
 *
 * Injected rather than imported so one board can serve two orderings. Each is
 * a TanStack mutation; `reorderMutationKey` is the key their in-flight state is
 * read under, which is what stops the resync effect from clobbering an
 * optimistic move.
 */
 ...
 */
export function useOrderingBoard<
  P extends OrderedItem & { rank: number },
  U extends OrderedItem,
>({
  data,
  ...
  data: OrderingData<P, U>
  writes: OrderingWrites
  search: string
  showUnrated: boolean
  unplacedSearch: string
}) {
  const sensors = useSortableSensors()
  const { place, reorder, unplace } = writes

  const pendingRankingCount = useMutationState({
    filters: { mutationKey: writes.reorderMutationKey, status: 'pending' },
  }).length

  // Common level data for any id, in either container.
  const itemsById = useMemo(() => {
    const m = new Map<string, P | U>()
  ...
  /**
   * Remove a placed entry from the ordering — it returns to the Unplaced panel.
   *
   ...
   */
  function removeFromOrdering(id: string) {
