...
const kept = (items: ListItem[], f = filters(), search = '') =>
...
describe('applyFilters', () => {
  ...
  describe('multi-select filters', () => {
    ...
    const pair = (matching: Partial<ListItem>) => {
    ...
    it.each([
      ...
      expect(
        kept(pair(matching as Partial<ListItem>), filters(filter as never))
      ).toEqual(['yes'])
    })
...
describe('sortItems', () => {
  const sorted = (items: ListItem[], sorts: SortSpec[], cats?: never) =>
