...
export function FilterTab({ open, onToggle, activeCount }: FilterTabProps) {
  return (
    <button
      ...
      aria-expanded={open}
      className={cn(
        'relative -mr-4 flex h-12 w-12 shrink-0 items-center justify-center rounded-l-xl border border-r-0 border-[var(--color-border-subtle)] bg-[var(--color-bg-surface)] text-text-secondary transition-colors hover:text-text-primary md:-mr-6',
    ...
    >
      ...
      {activeCount > 0 && !open && (
        <span className="absolute -left-1.5 -top-1.5 flex h-4 min-w-4 items-center justify-center rounded-full bg-[var(--color-primary)] px-1 text-[10px] font-bold text-white">
          {activeCount}
        </span>
      )}
    </button>
