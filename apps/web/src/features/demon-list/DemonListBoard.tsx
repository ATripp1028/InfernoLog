...
export function DemonListBoard({
  ...
  const {
    ...
    placedView,
    unplacedView,
  } = useDemonListBoard({ data, search, showUnrated, unplacedSearch })

  const PlacedColumn = (
    <PlacedDroppable>
      {filtering ? (
        <StaticPlaced entries={placedView} onRemove={removeFromDemonList} />
            ...
            {containers.placed.map((id, i) => {
              ...
              return item ? (
                <SortablePlacedRow
                  ...
                  onRemove={() => removeFromDemonList(id)}
                />
              ) : null
            })}
          </div>
...
function StaticPlaced({
  ...
  entries: ClassicDemonListResponse['placed']
...
function EmptyRanked() {
  return (
    <div className="flex min-h-[160px] items-center justify-center rounded-card border border-dashed border-border-subtle px-6 text-center text-sm text-text-tertiary">
      Drag a level here from Unplaced to start your demon list.
    </div>
  )
}
