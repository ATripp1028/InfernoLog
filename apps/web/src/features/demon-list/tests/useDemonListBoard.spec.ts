...
vi.mock('@/lib/api/demonList', () => ({
  usePlaceOnDemonList: vi.fn(),
  useReorderDemonList: vi.fn(),
  useRemoveFromDemonList: vi.fn(),
}))

const { usePlaceOnDemonList, useReorderDemonList, useRemoveFromDemonList } =
  await import('@/lib/api/demonList')
const { useDemonListBoard } = await import('../useDemonListBoard')
...
beforeEach(() => {
  ...
  reorderMutate = vi.fn()
  unplaceMutate = vi.fn()
  vi.mocked(usePlaceOnDemonList).mockReturnValue(
    stubMutation({ mutate: placeMutate })
  )
  vi.mocked(useReorderDemonList).mockReturnValue(
    stubMutation({ mutate: reorderMutate })
  )
  vi.mocked(useRemoveFromDemonList).mockReturnValue(
...
function render(
  ...
  const view = renderHook(
    ({ data }: { data: ClassicDemonListResponse }) =>
      useDemonListBoard({
        data,
        search: opts.search ?? '',
...
function drag(
  result: { current: ReturnType<typeof useDemonListBoard> },
  id: string,
  over: string,
  crossTo?: string
) {
...
describe('useDemonListBoard', () => {
  ...
  describe('removing a row with the X button', () => {
    it('unplaces it', () => {
      ...
      act(() => result.current.removeFromDemonList('b'))
    ...
    it('writes nothing for an id that is not placed', () => {
      ...
      act(() => result.current.removeFromDemonList('x'))
      act(() => result.current.removeFromDemonList('nope'))
    ...
    it('still removes while the board is filtering', () => {
      ...
      expect(result.current.filtering).toBe(true)
      act(() => result.current.removeFromDemonList('a'))
