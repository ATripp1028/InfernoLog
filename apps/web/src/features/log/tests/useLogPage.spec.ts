...
import { ATTEMPTS_DOMAIN, DATE_MIN_MS, type LogItem } from '../types'
...
const { useMyProgress, useDeleteProgress } = await import('@/lib/api/log')
...
const { useLogPage } = await import('../useLogPage')
...
beforeEach(() => {
  ...
  vi.mocked(useMyProgress).mockReturnValue(stubQuery<LogItem[]>({ data: [] }))
...
function render() {
  ...
  return renderHook(() => useLogPage(), { wrapper })
}

const withItems = (items: LogItem[]) =>
  vi
    .mocked(useMyProgress)
    .mockReturnValue(stubQuery<LogItem[]>({ data: items }))
  ...
  describe('the load gate', () => {
    it.each([
      ...
      [
        ...
        () =>
          vi
            .mocked(useMyProgress)
            .mockReturnValue(stubQuery<LogItem[]>({ isPending: true })),
      ],
    ...
    it('reports empty collections before anything loads', () => {
      vi.mocked(useMyProgress).mockReturnValue(
        stubQuery<LogItem[]>({ data: undefined })
      )
      const { result } = render()
