...
// The "Bloodbath · by Riot · At the Speed of Light  [Change]" header that sits
// atop every post-resolve step. "Change" returns to the find step.
  ...
  return (
    <div className="flex items-start justify-between gap-4">
      <div className="flex items-center gap-3">
        <div className="flex size-9 shrink-0 items-center justify-center rounded-md bg-[var(--color-primary-dim)] text-primary">
          {/* Placeholder for the level difficulty icon (GD asset). */}
          <span className="text-sm font-bold">{difficultyGlyph(level)}</span>
        </div>
        <div>
...
function difficultyGlyph(level: Level): string {
  if (!level.isDemon) return '★'
  return '☠'
}
