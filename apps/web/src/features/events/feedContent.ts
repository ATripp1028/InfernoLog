...
/** A 1-based demon-list position as the log writes it: `#4`, or "unplaced". */
 ...
 * many it dropped off the demon list altogether.
 */
...
/**
 * "Up 43 in your ranking" — the headline a rating change earns, or null
...
export function ratingRankHeadline(event: ActivityFeedEvent): string | null {
  ...
  if (before === null) return `New at #${after} in your ranking`
  const moved = before - after
  if (moved === 0) return null
  return `${moved > 0 ? 'Up' : 'Down'} ${Math.abs(moved)} in your ranking`
}
