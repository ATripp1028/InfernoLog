...
// Real DB; the demon list flow has no external HTTP, so only infra is mocked.
...
async function getRanking(userId: string): Promise<RankingBody> {
  const res = await get(userId, '/me/demon-list/classic')
...
// kind=COMPLETION update; placing it adds a ClassicDemonList row.
// ─────────────────────────────────────────────
...
async function indexOf(levelProgressId: string): Promise<number> {
  const row = await prisma.classicDemonList.findFirstOrThrow({
    where: { levelProgressId },
    select: { listIndex: true },
  })
  return Number(row.listIndex)
}
...
// ─────────────────────────────────────────────
// GET /me/demon-list/classic
// ─────────────────────────────────────────────

describe('GET /me/demon-list/classic', () => {
  it('returns placed entries ordered hardest-first with 1-based ranks', async () => {
    ...
    const res = await get(user.id, '/me/demon-list/classic')
...
// ─────────────────────────────────────────────
// POST /me/demon-list/classic — place
// ─────────────────────────────────────────────

describe('POST /me/demon-list/classic', () => {
  it('places the first entry into an empty ranking', async () => {
    const user = await seedUser(prisma)
    const lp = await seedCompletion(user.id)

    const res = await send(user.id, 'POST', '/me/demon-list/classic', {
      levelProgressId: lp.id,
    })
  ...
  it('drops at the top (no aboveId) and bottom (no belowId)', async () => {
    ...
    await send(user.id, 'POST', '/me/demon-list/classic', {
...
// ─────────────────────────────────────────────
// PATCH /me/demon-list/classic/:levelProgressId — reorder
// ─────────────────────────────────────────────

describe('PATCH /me/demon-list/classic/:levelProgressId', () => {
  it('moves a placed entry between new neighbours', async () => {
    ...
    const res = await send(user.id, 'PATCH', `/me/demon-list/classic/${c.id}`, {
      aboveId: a.id,
      belowId: b.id,
    })
...
// ─────────────────────────────────────────────
// DELETE /me/demon-list/classic/:levelProgressId — unplace
// ─────────────────────────────────────────────

describe('DELETE /me/demon-list/classic/:levelProgressId', () => {
  it('removes the demon list row and returns the entry to unplaced', async () => {
    const user = await seedUser(prisma)
    const lp = await seedPlaced(user.id, 1)

    const res = await send(user.id, 'DELETE', `/me/demon-list/classic/${lp.id}`)
    ...
    // The completion itself survives — only the ClassicDemonList row is gone.
    const lpRow = await prisma.levelProgress.findUnique({
      where: { id: lp.id },
    })
    expect(lpRow?.status).toBe('COMPLETED')
    expect(await prisma.classicDemonList.count()).toBe(0)
  })
  ...
  it('does not let a user unplace another user’s entry (404)', async () => {
    ...
    const res = await send(user.id, 'DELETE', `/me/demon-list/classic/${lp.id}`)
    expect(res.status).toBe(404)
    expect(await prisma.classicDemonList.count()).toBe(1)
  })
})
