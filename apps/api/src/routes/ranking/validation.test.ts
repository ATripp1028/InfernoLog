...
vi.mock('../../services/ranking', async (importOriginal) => {
  const actual = await importOriginal<typeof import('../../services/ranking')>()
  return {
    ...actual,
...
describe('ranking routes — body validation', () => {
  it('POST 400s on an unparseable body', async () => {
    ...
    const res = await send('POST', '/me/ranking/classic', '{oops')
  ...
  it('400s on a reorder body whose neighbour is the wrong type', async () => {
    const res = await send('PATCH', `/me/ranking/classic/${LP_ID}`, {
      aboveId: 42,
    })
