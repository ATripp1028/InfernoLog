 ...
 *
 * These drive the actual write paths — the ranking endpoints, the progress-edit
 ...
 * match what the ranking actually holds afterwards.
 *
 * The reconstruction-integrity sweep ("every current rankingIndex is the latest
...
describe('ranking placement', () => {
  it('records the mover with its real index and its new position', async () => {
    ...
    const res = await send(rankingApp, user.id, 'POST', '/me/ranking/classic', {
    ...
    const event = await onlyEvent(user.id, 'RANKING_PLACEMENT')
    ...
    expect(Number(mover.rankingIndex)).toBe(6)
...
describe('ranking unranking', () => {
  it('records a null positionAfter and the index the level last held', async () => {
    ...
    const event = await onlyEvent(user.id, 'RANKING_UNRANKED')
    ...
    expect(Number(mover.rankingIndex)).toBe(20)
  })
  ...
  it('fires when deleting the completion walks the entry out of COMPLETED', async () => {
    // The indirect path. A rankingIndex that vanishes without an event is a
...
describe('ranking rebalance', () => {
  it('records every level’s new index when a tight gap forces renormalisation', async () => {
    ...
    const rebalance = await onlyEvent(user.id, 'RANKING_REBALANCE')
    ...
    expect(Number(byLevel.get(above.inGameId)!.rankingIndex)).toBe(2)
    expect(Number(byLevel.get(below.inGameId)!.rankingIndex)).toBe(1)
...
describe('deleting an entry', () => {
  it('takes that level’s own event history with it', async () => {
    ...
    await send(rankingApp, user.id, 'DELETE', `/me/ranking/classic/${lp.id}`)
