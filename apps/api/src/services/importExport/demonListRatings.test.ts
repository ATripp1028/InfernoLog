...
const { commitImportRanking } = await import('./demonList')
...
const tx = {
  classicDemonList: {
    deleteMany: vi.fn(),
    createMany: vi.fn(),
    // The before/after snapshots the demon list replace emits its
    // DEMON_LIST_REBALANCE from.
    findMany: vi.fn(),
  },
...
/** The levelProgressIds written to the demon list, hardest first. */
function rankedLpIds(): string[] {
  const call = tx.classicDemonList.createMany.mock.lastCall
  return call
...
describe('commitImportRanking', () => {
  ...
  it('replaces the demon list with the sheet order, hardest first', async () => {
    completed([
  ...
  it('gives the hardest entry the highest index', async () => {
    ...
    const { data } = tx.classicDemonList.createMany.mock.lastCall![0] as {
      data: { listIndex: unknown }[]
    }
    expect(data.map((d) => Number(d.listIndex))).toEqual([2, 1])
  })
