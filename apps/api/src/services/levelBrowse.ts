...
import type { LevelBrowseQuery, LevelBrowseResult, LevelSort } from '@infernolog/core'
...
function sortDef(sort: LevelSort, q: string, searchBy: string): SortDef {
  ...
  switch (sort) {
    case 'relevance':
      return { expr: Prisma.sql`(${rel})::float8`, dir: 'DESC', type: 'num' }
    case 'likes':
      return { expr: Prisma.sql`(COALESCE("likes", -1))::float8`, dir: 'DESC', type: 'num' }
    case 'downloads':
      return { expr: Prisma.sql`(COALESCE("downloads", -1))::float8`, dir: 'DESC', type: 'num' }
    case 'stars':
      return { expr: Prisma.sql`(COALESCE("stars", -1))::float8`, dir: 'DESC', type: 'num' }
    case 'objectCount':
      return { expr: Prisma.sql`(COALESCE("objectCount", -1))::float8`, dir: 'DESC', type: 'num' }
    case 'recentlyRated':
      return {
    ...
    case 'name':
      return { expr: Prisma.sql`LOWER(COALESCE("name", ''))`, dir: 'ASC', type: 'text' }
  }
}
...
export async function browseLevels(
  ...
  if (cursor) {
    ...
    if (dec) {
      const cmp = s.dir === 'DESC' ? Prisma.sql`<` : Prisma.sql`>`
      const vparam =
  ...
  const whereSql = conds.length
    ...
    : Prisma.empty
  const dirSql = s.dir === 'DESC' ? Prisma.sql`DESC` : Prisma.sql`ASC`
